    ...
    function page($page){
        ...
        if(is_numeric($id)){
            ...
            if($a->num_rows >0) {
                ...
                $htmlOut .= '<div class="row mb-5" style="padding-top: 80px;">';
...
' . $imgThumb . '
                ...
                $htmlOut .= '<h1>' . $b["manufacturer"] . ' ' . $b["model"] . '</h1>';
                if ($b["price"] != null) {
                    $htmlOut .= '<div style="font-size: 22px; background: #efefef; padding: 5px;"><strong>Only: </strong><span>$' . number_format($b["price"], 2) . '</span></div>';
                } else {
                    $htmlOut .= '<span style="font-size: 20px;">Price: <span style="color: var(--bcss-secondary);">$'.number_format($b["single_price"],2) .'</span></span>';
                }
                $htmlOut .= '<h3 class="mt-3">Specifications</h3>';
                ...
                $htmlOut .= '<tr><td><strong>Location:</strong></td><td><a href="' . $b["city"] . '">' . $b["city"] . '</a></td></tr>';
                $htmlOut .= '</table>';
               ...
               $htmlOut .= '</table>';*/
                ...
                if($formTok["form_token"] != ''){
                    ...
                    /*$htmlOut .= '<form action="https://sunsouth.com/used-equipment-info-request" method="GET">
                        <input type="hidden" name="equipment_link" value="'.$actual_link.'" />
                        <input type="hidden" name="equipment_title" value="John Deere - '.$b["model"].'-'.$b["city"].'" />
                        <input type="hidden" name="equipment_location" value="'.$b["city"].'" />
                        <input class="btn btn-success" type="submit" value="REQUEST MORE INFO" />
                      </form><br><br>';*/
                }
...
<!-- AddToAny END -->';
                ...
                $content[] = array("page_name" => $page, "page_title" => $page, "page_content" => '<div class="container">' . $htmlOut . '</div>', "active" => true, "created" => time(), "last_edit" => '', "last_user" => '', "page_lock" => 'none', "page_type" => '', "page_desc" => '', "check_out" => false, "check_out_date" => '', "page_js" => '');
