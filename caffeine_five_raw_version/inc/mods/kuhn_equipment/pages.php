<?php
class kuhn_equipment
{
    function page($page)
    {
        include('inc/config.php');
        $a = $data->query("SELECT * FROM kuhn_equipment WHERE title = '$page'") or die($data->error);
        if ($a->num_rows > 0) {
...
<label>Your Review</label><br>
                        ...
                        <input type="hidden" name="eqiptype" id="eqiptype" value="kuhn">
            ...
            $html .= '<div class="col-md-8" style="padding:0"><img style="width:100%;" src="img/Kuhn/' . $image . '"></div>';
            ...
            if ($price != null) {
                if (isset($saveFlag)) {
                    ...
                    $saveButton = '<button class="btn btn-default save-later" data-eqid="' . $obj["id"] . '" data-eqname="' . $page . '" data-eqtype="kuhn" data-price="' . str_replace(",", "", $price) . '" data-url="' . $url . '"><i class="fa fa-shopping-cart"></i> Add To Cart</button>';
                }
            } else {
                $saveButton = '';
            }
            ...
            $html .= '<div class="col-md-4"><h1 class="eq_title">' . $tt . '<br><span class="sub-h1">' . $outTitleSub . '</span></h1>' . $description1 . '<br><br> <button class="btn btn-success moreinfo" data-url="'.$actual_link.'" data-equipment="Kuhn - '.$title.'" data-toggle="modal" data-target="#exampleModal">Request A Quote</button> ' . $saveButton . '<br><br>
<!-- AddToAny BEGIN -->
...
<!-- AddToAny END --><br><br><div style="font-size:30px">' . $starsNow . ' <br>' . $readRevs . ' <a style="font-size: 12px" href="javascript:void(0)" class="reviewit" data-neweqids="' . $obj["id"] . '"></a></div><br><br>' . $optLinksOut . '<br><br><div class="offer-titles-set"></div></div>';
            ...
            if ($obj["videos_active"] == 'true') {
                ...
                if (!empty($videos)) {
                    ...
                    if (count($videos) > 1) {
                        ...
                        $html .= '<div class="vidoverall" style="padding:20px; display:none;"><button type="btn" class="close close-vids" style="padding:10px"><i class="fa fa-times"></i> Close</button><div class="clearfix"></div><div class="embed-responsive embed-responsive-16by9"><iframe id="kuhnvids" class="embed-responsive-item" src="https://www.youtube.com/embed/' . $firstVid . '?rel=0" allowfullscreen></iframe></div></div>';
                        $html .= '</div>';
                        ...
                        if (!empty($vidOut)) {
                            ...
                            ///echo "This is ". $vids;
                            $data->query("UPDATE kuhn_equipment SET video_storage = '" . $data->real_escape_string($vids) . "' WHERE id = '" . $obj["id"] . "'") or die($data->error) or die($data->error);
                        } else {
                        ...
                        $html .= '<h2 style="color: #fff; padding: 25px">' . $videosTitle . '</h2>';
                        $html .= '<div class="col-md-12" style="padding: 0">';
                        $html .= '<div class="embed-responsive embed-responsive-16by9"><iframe id="kuhnvids" class="embed-responsive-item" src="https://www.youtube.com/embed/' . $firstVid . '?rel=0" allowfullscreen></iframe></div>';
                        $html .= '</div>';
                        $html .= '<div class="col-md-12" style="padding: 0; text-align: center">';
           ...
           /// $html .= '<div style="text-align: center"><button class="btn btn-default open-spec" style="margin: 20px"><i class="fa fa-align-left" aria-hidden="true"></i> Show More</button></div>';
            ...
            if ($obj["offers_active"] == 'true') {
                ...
                if ($lastTime > 10) {
                    ...
                    foreach ($finnalyOfffers as $offerlink) {
                        ...
                        $offerLinkClean = str_replace('/html/kuhn/us/en/website/', 'en/', $offerlink);
                        $offersLinkNow = 'https://www.kuhn.com/' . $offerLinkClean . '/index.json';
                        ...
                        if (is_array($objOffers) && $objOffers["Page"]["special-offers"]["ESIFragments"] != null) {
                            if (strpos($objOffers["Page"]["special-offers"]["ESIFragments"], 'https://www.kuhn.com') !== false) {
                                $jsonOfferOut = $this->file_get_contents_curl($objOffers["Page"]["special-offers"]["ESIFragments"]);
                            } else {
                                $jsonOfferOut = $this->file_get_contents_curl('https://www.kuhn.com' . $objOffers["Page"]["special-offers"]["ESIFragments"]);
                            }

                            //var_dump($objOffers["Page"]["special-offers"]["ESIFragments"]);
                            $content = str_replace('srcset="', 'srcset="https://kuhn.com', $jsonOfferOut);
                    ...
                    if ($offerZZ != null) {
                        ...
                        $data->query("UPDATE kuhn_equipment SET offers_storage = '" . $data->real_escape_string($offerZZa) . "', last_updated = '" . time() . "' WHERE id = '" . $obj["id"] . "'") or die($data->error);
            ...
            if ($obj["access_active"] == 'true') {
                ...
                if ($lastTime > 30) {
                    if ($obj["accessories"] != 'https://www.kuhn.com') {
                        ...
                        $data->query("UPDATE kuhn_equipment SET accessories_storage = '" . $data->real_escape_string($assOut) . "', last_updated = '" . time() . "' WHERE id = '" . $obj["id"] . "'") or die($data->error);
...
//            }
            ...
            $a = $data->query("SELECT * FROM kuhn_pages WHERE page_name = '$page' AND active = 'true'")or die($data->error);
            ...
            if($a->num_rows > 0){
                ...
                $categoryOut = preg_replace_callback('#{prodcat}data{/prodcat}#', function($match) use($page){
                    ...
                    include('inc/config.php');
                    $a = $data->query("SELECT * FROM kuhn_pages WHERE page_name = '$page'")or die($data->error);
                    $b = $a->fetch_array();
                    ...
                    for($i=0; $i<count($equip_content); $i++){
                        if($equip_content[$i]["type"] == 'category'){
                            ...
                            $c = $data->query("SELECT * FROM kuhn_pages WHERE page_name = '".$equip_content[$i]["title"]."'")or die($data->error);
                            $d = $c->fetch_array();
            ...
            <div class="product-grid6">
                <div class="product-image6">
                    <a href="'.$_SERVER['REQUEST_URI'].'/'.$d["page_name"].'">
                        <img class="pic-1" src="'.$d["cat_img"].'">
                    </a>
                </div>
                <div class="product-content">
                    <h3 class="title"><a href="'.$_SERVER['REQUEST_URI'].'/'.$d["page_name"].'">'.str_replace('-',' ', $d["page_name"]).'</a></h3>
                    <small style="padding: 3px">'.$d["page_desc"].'</small>
                    
                </div>
                <div class="social">
                   <span style="display: block; background: #fee72f; color: #333; font-weight: bold; margin: 1px; padding: 3px">'.str_replace('-',' ', $d["page_name"]).'</span>
                    <small style="padding: 3px; display: block; background: #fff; color: #333; height: auto">'.$d["page_desc"].'<br><br></small>
                            ...
                            $e = $data->query("SELECT * FROM kuhn_equipment WHERE title = '".$equip_content[$i]["title"]."'")or die($data->error);
                            $f = $e->fetch_array();
                            ...
                            if($f["price"] != null){
                                $priceOuts = '<span class="product-new-label" style="color: #333; font-weight:bold; font-size: 14px">Only $'.$f["price"].'</span>';
                            }
    ...
    <div class="product-grid-deerepro">
        <div class="product-image">
            <a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">
                <img class="pic-1" src="../img/Kuhn/' . $mainImage . '">
        ...
        <div class="product-content">
            <h3 class="title"><a href="' . $_SERVER['REQUEST_URI'] . '/' . $f["title"] . '">' . $f["title"] . '</a></h3>
