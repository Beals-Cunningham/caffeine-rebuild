...
CONCAT ('manufacturer', 'category') LIKE '%$searchInput%' $totSql AND active = 'true'");
    ...
    if($lastpage > 1)
    {
        ...
        $pagination .= '<div class="btn-group flex-wrap" role="group" aria-label="Basic example">';
        //previous button
        if ($page > 1)
            //$pagination.= "<a href=\"videos-$prev.html\">« previous</a>";
            $pagination .= '<button type="button" class="btn btn-warning previ px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;" onclick="pageData(\''.$prev.'\')"><i class="fa fa-chevron-left"></i> Previous</button><button type="button" class="btn btn-secondary px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;" onclick="pageData(\'1\')">First Page</button>';

        else
            //$pagination.= "<span class=\"disabled\">« previous</span>";
            $pagination .= '<button type="button" class="btn btn-warning disabled px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;">Previous</button>';

        $pagination .= '<ul class="p-0 m-0 list-unstyled d-flex align-items-center" style="color: #FAB906;">';
        if ($page == 1){
            $pagination .= '<li class="mx-2" onclick="pageData(\''.$page.'\')">'.$page.'</li>';
            $pagination .= '<li class="mx-2" onclick="pageData(\''.($page+1).'\')">'.($page+1).'</li>';
            $pagination .= '<li class="mx-2" onclick="pageData(\''.($page+2).'\')">'.($page+2).'</li>';
        ...
        if ($page < $lastpage)
            ...
            $pagination .= '<button type="button" class="btn btn-warning px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;" onclick="pageData(\''.$lastpage.'\')">Last Page</button><button type="button" class="btn btn-secondary px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;" onclick="pageData(\''.$next.'\')">Next <i class="fa fa-chevron-right"></i></button>';
        else
            //$pagination.= "<span class=\"disabled\">next »</span>";
            $pagination .= '<button type="button" class="btn btn-warning disabled px-3 m-2" style="background: #185733; color: #FAB906; border-color: #FAB906;">Next</button>';
        $pagination.= "</div>";
    }
...
//           $theOptions .= '<li class="col">'..'</li>';
                ...
                <div class="align-items-center align-content-right col-md-3 border-left mt-1">
                   ...
                   $unitOutput .= '<div class="col-sm-12 col-md-6 col-lg-4 col-xl-3">
                                      <a style="text-decoration: none; color: #333" href="'.$actual_link.'/'.str_replace(' ', '-',$b[" manufacturer"]).'/'.str_replace(' ', ' -',$b["category"]).'/'.str_replace(' ', ' -',$b["model"]).'/'.$b["id"].'">
                                        <div class="card mb-4 shadow-sm" style="background-image: url('.$mainImg.'); background-size: cover; background-position: top; background-repeat: no-repeat; border-radius: 0;">
                                          <div class="position-relative"> 
                                              <img src="img/spacer.png">
                                              <div class="used-price d-flex justify-content-center"> 
                                                <h5 class="d-inline-block mb-0 py-2">$'.number_format($b["single_price"]).'</h5> 
                                              </div> 
                                          </div>
                                            ...
                                            <div class="row">
                                              <div class="col" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
                                                  <div class="p-2" style="background: white;"> 
                                                      <h5 class="">'.$b["manufacturer"].' '.$b["model"].'</h5> 
                                                      <ul class="p-0 m-0 list-unstyled"> 
                                                          <li><b>YEAR</b> &gt;&gt; '.$modYear.'</li> 
                                                          <li><b>STOCK</b> # &gt;&gt; '.$b["stockNumber"].'</li> 
                                                          <li><b>LOCATION</b> &gt;&gt; '.$b["city"].'</li> 
                                                      </ul> 
                                                  </div>
                                              </div>
                                            </div>
