...
<br>
    ...
    function getLocationSmall($id){
        ...
        $location_name = '<h3 class="location-title mt-3">'.$b["location_name"].', '.$b["location_state"].'</h3>';
        $address = $b["location_address"].'<br>'.$b["location_city"].', '.$b["location_state"].' '.$b["location_zip"].'';
        $addressText = '<p class="py-0 mb-2" style="text-align: center; font-size: .8rem; color: var(--bcss-secondary);">'.$b["location_address"].'<br>'.$b["location_city"].', '.$b["location_state"].' '.$b["location_zip"].'</p>';
        ...
        $addresslinkNew = '<a href="http://maps.google.com/?q='.$address.'">'.$addressText.'</a>';

        $phoneJson = json_decode($b["location_phones"],true);
        $phoneName = $phoneJson[0]["phoneName"];
        $phoneNum = $phoneJson[0]["phoneNum"];

        if($phoneJson[0]["phoneClass"] != ''){
            $phoneClass = 'class="'.$phoneJson[0]["phoneClass"].'"';
        }else{
            $phoneClass = '';
        }
        $phonelink = '<a '.$phoneClass.' href="tel:'.$phoneNum.'">'.$phoneNum.'</a>';

        $phone = '<p class="py-0 mb-2" style="text-align: center; font-size: .8rem; color: var(--bcss-secondary);">MAIN <i class="fa fa-angle-double-right" aria-hidden="true"></i> ' .$phonelink. '</p>';

        $html = '<div class="col-6 col-md-4 px-3 mb-3 d-flex flex-column align-items-center">
                     <a href="Locations/'.$b["location_link"].'">
                        <img src="'.$b["location_img"].'" alt="'.$b["location_name"].', '.$b["location_state"].'" class="img-responsive" style="width: 160px; height: 160px; object-fit: cover; object-position: center center; border-radius: 50%; border: 5px solid var(--bcss-secondary);" />
                     </a>
                     '.$location_name.'
                     '.$addresslinkNew.'
                     '.$phone.'
                     <div class="row">
                        <a class="btn-more_info mr-2" href="Locations/'.$b["location_link"].'">More Info</a>
                        <a class="btn-directions" href="https://www.google.com/maps/place/'.$googleadd.'" target="_blank">Directions</a>
                     </div>
                 </div>';
    ...
    function getLocationPage($id){
        ...
        if($b["form_code"] != null){
            $formCode = '<p class="mt-2" style="background: var(--bcss-secondary); padding: 10px; font-size: 20px; font-weight: bold; color: #fff;">Contact Us</p>
                        <div style="background: #efefef; padding: 20px; margin-top: -15px;">'.$b["form_code"].'</div>';
        }else{
            $formCode = '';
        }

        if($b["location_img"] != null){
            $locImg = '<img src="'.$b["location_img"].'" alt="" class="img-responsive" style="width:100%" /><br />';
        }else{
            $locImg = '';
        }

        $mapOut = '<div class="mt-2" style="background:#efefef"><div style="width:20px; height:20px; position:absolute; left:50%; top:50%;"></div>'.$b["location_map"].'</div>';
  ...
  <div class="panel panel-success">
   <div class="panel-heading" style="background: var(--bcss-secondary)">
    <h3 class="panel-title p-2">Location Info</h3>
   </div>
   <div class="panel-body">
    <div class="row">
        <div class="col-md-9 col-6">'.$addresslinkNew.'</div>
        <div class="col-md-3 col-6"><a class="my-auto" href="order-parts" target="_blank"><button class="btn btn-success" style="color: var(--bcss-active);">Order Parts Online</button></a></div>
    </div>
    <br /><br /> <strong>Phone Numbers</strong>';
     ...
     <tbody>';
       ...
       <td style="text-align: right;"><a '.$phoneClass.' href="tel:'.$phonAes[$i]["phoneNum"].'">'.$phonAes[$i]["phoneNum"].'</a></td>
      </tr>';
        }
...
//            echo $theImage;
            ...
            $html .= '<div class="col-md-6 col-lg-4 col-xl-3 mb-5 mt-5">
                        <div class="position-relative">
                            ...
                            <div class="used-price d-flex justify-content-center">
                                <h5 class="d-inline-block mb-0 py-2">$ '.number_format(floatval($b["single_price"])).'</h5>
                            </div>
                        </div>
                        <div class="p-2" style="background: white; height: 110px;">
                            <h5><a href="'.$link.'">'.$modelInfo.'</a></h5>
                            <ul class="p-0 m-0 list-unstyled">';
                            if($b["modelYear"] != '') {
                                $html .= '<li><b>YEAR</b> >>> '.$b["modelYear"].'</li>';
                            }else{
                                $html .= '';
                            }
                            if($b["stockNumber"] != '') {
                                $html .= '<li><b>STOCK #</b> >>> '.$b["stockNumber"].'</li>';
                            }else{
                                $html .= '';
                            }
                            if($b["city"] != '') {
                                $html .= '<li><b>LOCATION</b> >>> '.$b["city"].'</li>';
                            }else{
                                $html .= '';
                            }
   ...
   <div class="item-info">
       <div class="item-info-header">
           <h3 style="min-height: 42px;"><a href="'.$link.'">'.$modelInfo.'</a></h3>
    ...
    function mailItReply($toArs,$fromemail,$fromName,$subject,$message){
        // Import the Postmark Client Class:
        include('config.php');
