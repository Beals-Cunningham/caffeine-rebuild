...
$header = preg_replace_callback('#{bean}(.*?){\/bean}#',
    "bean_get_extra", $stringog);
$headerout = preg_replace_callback('#{include}(.*?){\/include}#',
    "caffeine_include_get", $header);
$headeroutfin = preg_replace_callback('#{nav}(.*?){\/nav}#',
    "caffeine_include_nav", $headerout);
...
$html = preg_replace_callback('#{mod}(.*?){/mod}#',
    "process_mod", $html);
...
$html0 = preg_replace_callback('#{bean}(.*?){\/bean}#',
    "bean_get", $html);
...
$html00 = preg_replace_callback('#{bean}(.*?){\/bean}#',
    "bean_get_extra", $html0);
...
$htmlout = preg_replace_callback('#{include}(.*?){\/include}#',
    "caffeine_include_get", $html00);
...
$htmlgo = preg_replace_callback('#{nav}(.*?){\/nav}#',
    "caffeine_include_nav", $htmlout);
...
$htmlgo = preg_replace_callback('#{locsmall}(.*?){/locsmall}#',
    "location_get", $htmlgo);
...
$htmlgo = preg_replace_callback('#{locpage}(.*?){/locpage}#',
    "location_get_page", $htmlgo);
...
$htmlgo = preg_replace_callback('#{form}(.*?){/form}#',
    "form_get", $htmlgo);
...
$htmlgo = preg_replace_callback('#{usedfeatured}MFv30{/usedfeatured}#',
    "getusedfet", $htmlgo);
...
$string = ob_get_clean();
$footer = preg_replace_callback('#{bean}(.*?){\/bean}#',
    "bean_get_extra", $string);
$footerout = preg_replace_callback('#{include}(.*?){\/include}#',
    "caffeine_include_get", $footer);
$footeroutfin = preg_replace_callback('#{nav}(.*?){\/nav}#',
    "caffeine_include_nav", $footerout);

$footeroutfin = preg_replace_callback('#{form}(.*?){/form}#',
    "form_get", $footeroutfin);
...
//$output = preg_replace('/(<script[^>]*>.+?<\/script>|<style[^>]*>.+?<\/style>)/is', '', $output);
