...
class kuhnClass
{
    ...
    function getProduct($prod, $cat)
    {
        ...
        $a = $data->query("SELECT * FROM kuhn_equipment WHERE title = '$prod'");
        $b = $a->fetch_array();
    ...
    function getcats($row)
    {
        ...
        $a = $data->query("SELECT * FROM kuhn_equipment WHERE $row != '' AND active = 'true' GROUP BY $row");
    ...
    function getEquipmentProducts($cattype, $catname)
    {
        ...
        if ($catname != '') {
            ...
            $checkRun = $data->query("SELECT * FROM kuhn_equipment WHERE active = 'true' AND $cattype = '$catname'");
            $che = $checkRun->fetch_array();

            if ($che["$cattype1"] == '') {
                $a = $data->query("SELECT * FROM kuhn_equipment WHERE active = 'true' AND $cattype = '$catname'");
            } else {
                $a = $data->query("SELECT * FROM kuhn_equipment WHERE active = 'true' AND $cattype = '$catname' GROUP BY $cattype1");
            }
            ...
            $a = $data->query("SELECT * FROM kuhn_equipment WHERE active = 'true' GROUP BY parent_cat");
            while ($b = $a->fetch_array()) {
    ...
    function getBackLink($cattype, $catname)
    {
        ...
        //echo $cattype .' - '. $catname;
        $a = $data->query("SELECT * FROM kuhn_equipment WHERE $cattype = '$catname'");
        $b = $a->fetch_array();
    ...
    function getDeerePage($name){
        ...
        $a = $data->query("SELECT * FROM kuhn_pages WHERE id = '$name'");
        $b = $a->fetch_assoc();
        return $b;
    }
    ...
    function getEqCats($filter)
    {
        ...
        $a = $data->query("SELECT * FROM kuhn_pages WHERE active = 'true' $addSql ORDER BY page_name ASC") or die($data->error);
        while ($b = $a->fetch_array()) {
    ...
    function getPagesCat($line){
        ...
        $ars = array();
        $a = $data->query("SELECT * FROM kuhn_pages WHERE active = 'true' ORDER BY page_name ASC");
        while($b = $a->fetch_array()){
    ...
    function checkPages(){
        ...
        $a = $data->query("SELECT * FROM kuhn_pages WHERE active = 'true'");
    ...
    function getSetupPages(){
        ...
        $url = 'http://caffeinerde.com/plugins/kuhn_equipment/pageprocess.php';
...
// use key 'http' even if you send the request to https://...
    ...
    function checkUpdates(){
        ...
        $a = $data->query("SELECT * FROM beans WHERE bean_folder = 'kuhn_plugin'");
        $b = $a->fetch_array();


        $url = 'http://caffeinerde.com/plugins/kuhn_equipment/updates.php';
        $key = 'IBN72234957UTYGALAXY!@';
        $password = 'YELLOANDGREENMAKEBLUE434';
